...
describe('/v1/caches/cloudfront', () => {
  ...
  describe('/zones', () => {
    describe('DELETE', () => {
      ...
      it('should reply "Bad request" when invalid payload is sent', () => {
        cloudfrontAPIMock.post('/2020-05-31/distribution/abcd/invalidation').times(0);
    ...
    describe('DELETE', () => {
      ...
      it('should flush more than 1 URL of the Cloudfront cache', () => {
        cloudfrontAPIMock
          .post(
            '/2020-05-31/distribution/abcd/invalidation',
            '<InvalidationBatch xmlns="http://cloudfront.amazonaws.com/doc/2020-05-31/"><Paths><Quantity>2</Quantity>' +
  ...
  describe('/zones/${distribution_id}/directories', () => {
    describe('DELETE', () => {
      let flushRequest: request.Test;

      beforeEach(() => {
        flushRequest = request(server.listener)
          .delete('/v1/caches/cloudfront/zones/abcd/directories')
          .set('accept', 'application/json');
      });

      it('should flush a directory of the Cloudfront cache', () => {
        cloudfrontAPIMock
          .post(
            '/2020-05-31/distribution/abcd/invalidation',
            '<InvalidationBatch xmlns="http://cloudfront.amazonaws.com/doc/2020-05-31/"><Paths><Quantity>1</Quantity>' +
              '<Items><Path>/folder/*</Path></Items></Paths>' +
              '<CallerReference>0</CallerReference></InvalidationBatch>'
          )
          .reply(
            200,
            `<?xml version="1.0"?>\n<Invalidation xmlns="http://cloudfront.amazonaws.com/doc/2018-11-05/"><Id>abcd` +
              `</Id><Status>InProgress</Status><CreateTime>2019-07-24T08:47:53.726Z</CreateTime><InvalidationBatch>` +
              `<Paths><Quantity>1</Quantity><Items><Path>/folder/*</Path></Items></Paths>` +
              `<CallerReference>${Date.now().toString()}</CallerReference></InvalidationBatch></Invalidation>`
          );
        ...
        return new Promise((resolve, reject) => {
          flushRequest
            .send({
              awsAccessKeyID: 'access key ID',
              awsSecretAccessKey: 'secret access key',
              directories: ['https://test-domain.com/folder'],
            })
            .expect('content-type', /application\/json/)
            .expect(200)
            .expect({
      ...
      it('should flush more than 1 URL of the Cloudfront cache', () => {
        cloudfrontAPIMock
          .post(
            '/2020-05-31/distribution/abcd/invalidation',
            '<InvalidationBatch xmlns="http://cloudfront.amazonaws.com/doc/2020-05-31/"><Paths><Quantity>2</Quantity>' +
              '<Items><Path>/folder1/*</Path><Path>/folder2/*</Path>' +
              '</Items></Paths><CallerReference>0</CallerReference></InvalidationBatch>'
          )
          .reply(
            200,
            `<?xml version="1.0"?>\n<Invalidation xmlns="http://cloudfront.amazonaws.com/doc/2018-11-05/"><Id>abcd` +
              `</Id><Status>InProgress</Status><CreateTime>2019-07-24T08:47:53.726Z</CreateTime><InvalidationBatch>` +
              `<Paths><Quantity>2</Quantity><Items><Path>/folder1/*</Path>` +
              `<Path>/folder2/*</Path></Items></Paths><CallerReference>` +
              `${Date.now().toString()}</CallerReference></InvalidationBatch></Invalidation>`
          );
      ...
      it('should reply "Bad request" when invalid payload is sent', () => {
        ...
        return new Promise((resolve, reject) => {
          flushRequest
            ...
            .expect(400)
            .expect({
              message: 'child "directories" fails because ["directories" is required]',
              validation: { source: 'payload', keys: ['directories'] },
            })
            .end((error: any, _: request.Response) => {
              if (error) {
                reject(error);
